...
function storeNewOrder(req, res, next) {
    ...
    const {
        ...
        products, total_price, shipping_price, billing_city, billing_country, billing_street
    } = data;

    // CAMPI OBBLIGATORI
    if (!name || !surname || !email || !city || !country || !street || !postal_code || !province || !billing_city || !billing_country || !billing_street) {
    ...
    connection.query(sqlCustomer, customerValues, (err, customerResult) => {
        ...
        connection.query(sqlInvoice, [newCustomerId, total_price, shipping_price, billing_street, billing_city, billing_country], (err, invoiceResult) => {
            if (err) return next(err);
            ...
            products.forEach((curProduct) => {
                ...
                connection.query(sqlSearchID, [curProduct.slug], (err, resultId) => {
                    productId = resultId[0].id;
                    connection.query(
                        productInvoiceSql,
                        [productId, newInvoiceId, curProduct.quantity, curProduct.price_at_purchase],
                        (err) => {
                            if (err && !hasError) {
                                hasError = true;
                                return next(err);
